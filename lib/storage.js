...
makeMailbox = function () {
    ...
    getStatus : function (path,cb) {
      var err, ret, mailbox = folderCache[path], seen, unseen, recent, total;
      if (mailbox) {
        err = null;
        total = mailbox.messages.length;
        seen = _.reduce(mailbox.messages,function (total,msg) {
      		return total + (_.contains(msg.flags||[],"\\Seen") ? 1 : 0);
      	},0), unseen = total - seen;
        recent = _.reduce(mailbox.messages,function (total,msg) {
      		return total + (_.contains(msg.flags||[],"\\Recent") ? 1 : 0);
      	},0);
        ret = {
          messages: total,
          recent: recent,
          unseen: unseen,
          uidnext: uidnextCache[path] || 1,
          uidvalidity: mailbox.uidvalidity || uidnextCache[path] || 1
        };
      } else {
        err = "Invalid folder";
      }
